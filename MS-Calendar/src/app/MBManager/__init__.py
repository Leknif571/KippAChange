...
RABBITMQ_CALENDAR_QUEUE = os.getenv("RABBITMQ_CALENDAR_QUEUE")
...
def publish_calendar_on_exchange(message: dict):
    try:
        connection = get_rabbitmq_connection()
        channel = connection.channel()
...
def create_calendar(calendar_data: dict):
    print(f"Création du pari avec les données : {calendar_data}", flush=True)
...
def start_consuming():
    def callback(ch, method, properties, body):
        ...
        time.sleep(2)
        create_calendar(body_received['data'])
        publish_calendar_on_exchange({"event": "calendar_created", "data": body_received['data']})

        

    connection = get_rabbitmq_connection()
    channel = connection.channel()
    result = channel.queue_declare(queue=RABBITMQ_CALENDAR_QUEUE, exclusive=False)
